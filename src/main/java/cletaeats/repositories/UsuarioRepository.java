...
public class UsuarioRepository {
    ...
    public void cargarDatosCliente(Usuario usuario) throws SQLException {
        String sql = "SELECT cedula, nombre, direccion, telefono, email FROM clientes WHERE usuario_id = ?";
        try (Connection conn = conexion.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
    ...
    public void cargarDatosRepartidor(Usuario usuario) throws SQLException {
        String sql = "SELECT cedula, nombre, direccion, telefono, email FROM repartidores WHERE usuario_id = ?";
        try (Connection conn = conexion.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, usuario.getId());
            try (ResultSet rs = stmt.executeQuery()) {
